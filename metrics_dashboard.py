...
min_dt = pd.to_datetime(df['created_at']).min()
max_dt = pd.to_datetime(df['created_at']).max()
...
col1.metric("Latência Média (s)", f"{df['latency'].mean():.3f}", delta=None, help="Média de todas as latências")
col2.metric("Latência Máxima (s)", f"{df['latency'].max():.3f}", delta=None, help="Maior latência registrada")
col3.metric("Latência Mínima (s)", f"{df['latency'].min():.3f}", delta=None, help="Menor latência registrada")
col4.metric("p95 (s)", f"{np.percentile(df['latency'], 95):.3f}", delta=None, help="95% das latências abaixo deste valor")
col5.metric("p99 (s)", f"{np.percentile(df['latency'], 99):.3f}", delta=None, help="99% das latências abaixo deste valor")
