...
st.markdown(
    "<h1 style='text-align: center; color: #4F8BF9;'>🚀 Dashboard de Latência dos LLMs</h1>",
    unsafe_allow_html=True,
)
st.markdown(
    "<p style='text-align: center; color: #888;'>Visualize métricas de latência por provedor em tempo real.</p>",
    unsafe_allow_html=True,
)
...
st.markdown(
    "<h3 style='color:#4F8BF9;'>🗓️ Filtro de Período (Data e Hora)</h3>",
    unsafe_allow_html=True,
)
...
end_date = col2.date_input(
    "Data final", value=max_dt.date(), min_value=min_dt.date(), max_value=max_dt.date()
)
...
st.markdown(
    "<h2 style='color:#4F8BF9;'>📊 Estatísticas Gerais</h2>", unsafe_allow_html=True
)
...
st.markdown(
    "<h2 style='color:#F98B4F;'>📊 Estatísticas por Provedor</h2>",
    unsafe_allow_html=True,
)
...
st.markdown(
    "<h4 style='color:#4F8BF9;'>📋 Tabela de Estatísticas por Provedor</h4>",
    unsafe_allow_html=True,
)
...
st.markdown(
    "<h2 style='color:#4F8BF9;'>📦 Boxplot de Latência - Todos os Provedores</h2>",
    unsafe_allow_html=True,
)
fig = px.box(
    df,
...
fig.update_layout(
    plot_bgcolor="#F5F5F5", paper_bgcolor="#F5F5F5", font=dict(color="#222")
)
st.plotly_chart(fig, use_container_width=True)


st.markdown(
    "<h2 style='color:#F98B4F;'>📦 Boxplot Individual por Provedor</h2>",
    unsafe_allow_html=True,
)
for provider, group in df.groupby("provider"):
    fig_ind = px.box(
        ...
        y="latency",
        points="all",
        title=f"Boxplot de Latência - {provider}",
        color_discrete_sequence=["#F98B4F"],
    )
    fig_ind.update_layout(
        plot_bgcolor="#FFF8F0", paper_bgcolor="#FFF8F0", font=dict(color="#222")
    )
    st.plotly_chart(fig_ind, use_container_width=True)
    st.markdown("<hr style='border:0.5px solid #F98B4F'>", unsafe_allow_html=True)

st.markdown(
    "<h2 style='color:#4F8BF9;'>⏱️ Latência por Requisição</h2>", unsafe_allow_html=True
)
...
st.markdown(
    "<h2 style='color:#4F8BF9;'>🎯 Percentis por Provedor</h2>", unsafe_allow_html=True
)
percentis = []
for provider, group in df.groupby("provider"):
    percentis.append(
...
st.markdown(
    "<h2 style='color:#4F8BF9;'>🔎 Dados Detalhados</h2>", unsafe_allow_html=True
)
st.dataframe(
    df.sort_values("created_at", ascending=False).style.background_gradient(
        cmap="Blues"
    ),
...
st.markdown("<hr style='border:1px solid #4F8BF9'>", unsafe_allow_html=True)
st.markdown(
    "<p style='text-align:center; color:#4F8BF9;'> UERJ 💙🧡</p>",
    unsafe_allow_html=True,
)
